...
<div class="card-body">
    ...
    <form method="POST" action="{{ route('update-question-category', $question_category) }}">
        @csrf
        @method('PATCH')
        ...
        <div class="form-group">
            ...
            <input id="name" name="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" 
                value="{{ $question_category->name }}" placeholder="Name" required autofocus>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" 
                row="3" required>{{ $question_category->description }}</textarea>
